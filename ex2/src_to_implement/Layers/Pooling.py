...
class Pooling(BaseLayer):
    def __init__(self, stride_shape, pooling_shape):
        ...
        self.stride_shape = stride_shape if isinstance(stride_shape, (list, tuple)) else (stride_shape, stride_shape)
        self.pooling_shape = pooling_shape if isinstance(pooling_shape, (list, tuple)) else (pooling_shape, pooling_shape)
            ...
            for channel in range(c):
                for oy in range(y_out):
                    for ox in range(x_out):
                        ...
                        window = input_tensor[batch, channel, start_y:end_y, start_x:end_x]
            ...
            for channel in range(c):
                for oy in range(y_out):
                    for ox in range(x_out):
                        ...
                        error_prev[batch, channel, in_y, in_x] += error_tensor[batch, channel, oy, ox]
